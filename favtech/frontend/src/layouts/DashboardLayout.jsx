...
const DashboardLayout = () => {
  ...
  const navGroups = [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'Help Hub', path: '/dashboard/help', icon: <HelpCircle className="w-5 h-5" /> },
      ]
    }
  ];
  ...
  return (
    <div className={`flex h-screen overflow-hidden ${darkMode ? 'bg-gray-950 text-white' : 'bg-[#f8fafc] text-[#0f172a]'} font-sans transition-colors duration-300`}>
      ...
      {/* ─── Mobile Header ─── */}
      <div className="lg:hidden fixed top-0 w-full h-20 bg-white/90 backdrop-blur-md z-[100] flex items-center justify-between px-5 border-b border-slate-100 shadow-sm">
      ...
      <aside className={`
        ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-[#0f172a]'} text-white flex flex-col flex-shrink-0 transition-transform duration-300 z-[120]
      ...
      `}>
        <div className={`p-6 border-b ${darkMode ? 'border-gray-800' : 'border-white/10'} flex-shrink-0 flex items-center justify-between`}>
          <div className="flex items-center gap-3">
            {settings.logo_url ? (
              <img src={`http://localhost:5000${settings.logo_url}`} alt="Logo" className="max-h-10 w-auto object-contain" />
            ) : (
              <div className="w-10 h-10 bg-white/10 rounded-2xl flex items-center justify-center flex-shrink-0 overflow-hidden">
                <TrendingUp className="w-5 h-5 text-teal-400" />
              </div>
            )}
          </div>
          {sidebarOpen && (
            <button onClick={() => setSidebarOpen(false)} className="lg:hidden p-2 hover:bg-white/5 rounded-xl transition-colors">
        ...
        <div className="px-4 pb-6 flex-shrink-0">
          <button
            ...
            className="flex items-center gap-3 px-4 py-3.5 w-full rounded-2xl text-red-400/50 hover:bg-red-500/10 hover:text-red-400 transition-all"
          >
            <LogOut className="w-5 h-5 flex-shrink-0" />
            <span className="text-[0.65rem] font-black uppercase tracking-widest">Terminate Session</span>
          </button>
        </div>
      </aside>

      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        <header className={`hidden lg:flex flex-shrink-0 h-20 border-b ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-slate-100'} items-center px-6 gap-6 z-30`}>
          <div className={`flex-1 flex max-w-md items-center gap-4 px-5 py-2.5 rounded-2xl border ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-slate-50 border-slate-100'}`}>
          ...
          <div className="flex items-center gap-3">
             <button onClick={() => setDarkMode(!darkMode)} className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${darkMode ? 'bg-gray-800 text-teal-400' : 'bg-slate-50 text-slate-400 hover:text-teal-500'}`}>
               {darkMode ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
             </button>

             <div className="relative">
                <button onClick={() => setNotifOpen(!notifOpen)} className={`w-10 h-10 rounded-xl flex items-center justify-center relative transition-all ${darkMode ? 'bg-gray-800 text-white' : 'bg-slate-50 text-slate-400 hover:text-teal-500'}`}>
                   <Bell className="w-5 h-5" />
                   <span className="absolute top-2.5 right-2.5 w-2 h-2 bg-teal-500 rounded-full border-2 border-white" />
                </button>
             </div>

             <div className="relative">
               <button onClick={() => setProfileOpen(!profileOpen)} className={`flex items-center gap-3 p-1.5 rounded-2xl transition-all ${darkMode ? 'bg-gray-800 hover:bg-gray-700' : 'bg-slate-50 hover:bg-slate-100'}`}>
               ...
               {profileOpen && (
                 <div className={`absolute right-0 top-14 w-64 rounded-3xl shadow-2xl border z-50 overflow-hidden ${darkMode ? 'bg-gray-900 border-gray-800 text-white' : 'bg-white border-slate-100 text-navy'}`}>
                    ...
                    <div className="p-2">
                       <button onClick={() => { navigate('/dashboard/profile'); setProfileOpen(false); }} className="w-full text-left px-5 py-3.5 text-[0.65rem] font-black uppercase tracking-widest rounded-xl hover:bg-slate-50">Identity Settings</button>
                       <button onClick={handleLogout} className="w-full text-left px-5 py-3.5 text-[0.65rem] font-black uppercase tracking-widest text-red-500 hover:bg-red-50 rounded-xl mt-2">Sign Out</button>
                    </div>
                 </div>
               )}
             </div>
        ...
        <main className={`flex-1 overflow-y-auto overflow-x-hidden ${darkMode ? 'bg-gray-950' : 'bg-[#f8fafc]'} custom-scrollbar pt-20 lg:pt-0 pb-20 lg:pb-0 w-full`}>
           <div className="p-4 md:p-8 lg:p-10 max-w-7xl mx-auto min-h-full">
              <Outlet context={{ darkMode, user }} />
           </div>
        </main>

        <div className="lg:hidden fixed bottom-0 w-full h-[72px] bg-white border-t border-slate-100 shadow-xl flex items-center justify-around px-2 z-[100] rounded-t-3xl">
          <button onClick={() => setSidebarOpen(true)} className="flex flex-col items-center justify-center w-12 h-12 text-slate-400 hover:text-navy">
            <Menu className="w-5 h-5 mb-1" />
            <span className="text-[0.55rem] font-black uppercase">Menu</span>
          </button>
          <button onClick={() => navigate('/dashboard/add-funds')} className="flex flex-col items-center justify-center w-12 h-12 text-slate-400">
            <Wallet className="w-5 h-5 mb-1" />
            <span className="text-[0.55rem] font-black uppercase">Wallet</span>
          </button>
          <div className="relative -top-6">
            <button onClick={() => navigate('/dashboard')} className="w-14 h-14 bg-teal-500 rounded-full flex items-center justify-center shadow-xl shadow-teal-500/30 border-4 border-white text-white">
              <LayoutDashboard className="w-6 h-6" />
            </button>
          </div>
          <button onClick={() => navigate('/dashboard/new-order')} className="flex flex-col items-center justify-center w-12 h-12 text-slate-400">
            <Package className="w-5 h-5 mb-1" />
            <span className="text-[0.55rem] font-black uppercase">Order</span>
          </button>
          <button onClick={() => navigate('/dashboard/profile')} className="flex flex-col items-center justify-center w-12 h-12 text-slate-400">
